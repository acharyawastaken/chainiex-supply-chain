...
router.get('/:id', authenticate, async (req, res) => {
  try {
    const [rows] = await db.query(
      'SELECT user_id, full_name, email, shipping_address, created_at FROM users WHERE user_id = ?',
      [req.params.id]
    );
...
router.patch('/:id', authenticate, async (req, res) => {
  try {
    ...
    await db.query(
      'UPDATE users SET full_name = COALESCE(?, full_name), shipping_address = COALESCE(?, shipping_address) WHERE user_id = ?',
